...
```html
<x-cauri-upload-image-single id="profile" :image="$image" wire:model="imageSingle" />
```
> It requires the use of a Livewire Component.
> There is a trait that can be used with your Livewire Component `\CauriLand\UserInterface\Components\UploadImageSingle`.
...
```html
@push('scripts')
    <x-cauri-pages-includes-compress-image-scripts />
@endpush
```
...
```js
mix
    .js('vendor/cauriland/foundation/resources/assets/js/compress-image.js', 'public/js/compress-image.js')
```
...
```html
@push('scripts')
    <x-cauri-pages-includes-crop-image-scripts />
@endpush
```
...
```js
mix
    .js('vendor/cauriland/foundation/resources/assets/js/crop-image.js', 'public/js/crop-image.js')
```

#### How to use
```html
<x-cauri-upload-image-single
    id="profile"
    :image="$image"
...
```html
<x-cauri-upload-image-collection id="media" :image="$imageCollection" wire:model="tempCollection" />
```
> It requires the use of a Livewire Component.
> There is a trait that can be used with your Livewire Component `\CauriLand\UserInterface\Components\UploadImageCollection`.
...
```html
<x-cauri-upload-image-collection
    id="media"
    ...
    sortable
/>
...
```php
<x-cauri-alert type="info" title="Title Override">
...
<div class="relative">
    <x-settings-dropdown button-class="icon-button w-10 h-10">
        <button class="settings-dropdown-entry">@lang('actions.start')<x-cauri-icon name="plus" size="xs" calss="ml-2" /></button>
        <button class="settings-dropdown-entry">@lang('actions.stop')<x-cauri-icon name="minus" size="xs" calss="ml-2" /></button>
        <button class="settings-dropdown-entry">@lang('actions.reboot')<x-cauri-icon name="reload" size="xs" calss="ml-2" /></button>
    </x-settings-dropdown>
</div>
```
...
> Remember to wrap the items in `<x-cauri-expandable-item>...</x-cauri-expandable-item>` component.

```blade
<x-cauri-expandable total="12">
    @foreach($items as $item)
        <x-cauri-expandable-item>
            ...
        </x-cauri-expandable-item>
    @endforeach
...
<x-cauri-font-loader src="https://fonts.googleapis.com/css2?family=Inter&display=swap" preconnect="https://fonts.gstatic.com">
```

```blade
{{-- an example with a google font, omitting the `&display=swap`. It'll be appended automatically for Google Fonts! --}}
<x-cauri-font-loader src="https://fonts.googleapis.com/css2?family=Inter" preconnect="https://fonts.gstatic.com">
```
...
<button class="button-secondary">
    <x-cauri-loader-icon class="w-7 h-7 text-white" path-class="bg-theme-primary-600" />
</button>
```
...
<x-cauri-tabbed>
    <x-slot name="tabs">
        <x-cauri-tab name="tab-1">One</x-cauri-tab>
        <x-cauri-tab name="tab-2">Two</x-cauri-tab>
        <x-cauri-tab name="tab-3">Three</x-cauri-tab>
    </x-slot>
...
```html
<x-cauri-tabbed x-data="{ menuToggle: false, closeMenu() {() => menuToggle = false} }">
    <x-slot name="tabsTrigger">
        <x-cauri-dropdown :init-alpine="false" dropdown-property="menuToggle">
            <x-slot name="button">
...
The parameter accepted by `<x-cauri-tabbed>`
| Parameter        | Description                                   | Required |
| ---------------- | --------------------------------------------- | -------- |
...
The parameter accepted by `<x-cauri-tab>`
| Parameter | Description         | Required |
| --------- | ------------------- | -------- |
| name      | The name of the tab | yes      |

The parameter accepted by `<x-cauri-tab-panel>`
| Parameter | Description                                                          | Required |
| --------- | -------------------------------------------------------------------- | -------- |
| name      | The name of the tab panel (it must be the same used for `x-cauri-tab`) | yes      |
...
```blade
<x-cauri-pages-includes-layout-head
    default-name="cauri.cm"
...
| Parameter            | Description                             | Required | Default Value               |
| -------------------- | --------------------------------------- | -------- | --------------------------- |
| default-name         | The name of the project (e.g. Deployer) | no       | `config('app.name', 'CAURI')` |
...
```blade
<x-cauri-pages-includes-layout-body cookie-domain="cauri.cm">
    <x-cauri-navbar ...>
        ...
    </x-cauri-navbar>

    <x-slot name="content">
        <x-cauri-pages-includes-layout-content>
            <div>
                ...
            </div>
...
</x-cauri-pages-includes-layout-body>
```
...
| footer    | Slot used to override footer             | no       | `<x-cauri-footer />`                                     |

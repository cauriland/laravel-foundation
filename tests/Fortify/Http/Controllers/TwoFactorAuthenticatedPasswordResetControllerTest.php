...
use CauriLand\Foundation\Fortify\Http\Requests\TwoFactorResetPasswordRequest;
...
it('shows the two auth challenge form', function () {
    ...
    expect($response->getName())->toBe('cauri-fortify::auth.two-factor-challenge');
...
it('shows the reset form after validating the token', function () {
    ...
    expect($response)->toBeInstanceOf(View::class);
    expect($response->getName())->toBe('cauri-fortify::auth.reset-password');
});

    ...
    <div @class([
        ...
        @if (!$image && !$readonly)
            ...
            >
                <div class="text-theme-primary-500">
                    <x-cauri-icon name="cloud-arrow-up" :size="$iconSize"/>
                </div>
        ...
        @unless($readonly)
            ...
            <div x-show="isUploading" x-cloak>
                <x-cauri-loading-spinner class="right-0 bottom-0 left-0 rounded-xl" :dimensions="$dimensions"/>
            </div>
        @endunless
    </div>
    ...
    >
        ...
        @slot('buttons')
            ...
            <button type="button" class="{{ $cropSaveButtonClass }}" @click="Livewire.emit('saveCroppedImage')" dusk="crop-save-button">
                @if($cropSaveIcon)
                    <x-cauri-icon :name="$cropSaveIcon" size="sm" class="inline my-auto mr-2"/>
                @endif

    ...
    <div class="flex flex-col">
        <div class="flex flex-row justify-between mt-4 mb-2 w-full text-base font-semibold sm:px-6">
            <div class="flex relative flex-row space-x-2 sm:static">
                    ...
                    <button type="button" class="flex justify-center items-center w-10 h-10 rounded border border-solid cursor-pointer focus:outline-none text-theme-secondary-400 border-theme-secondary-200 dark:border-theme-secondary-800 hover:text-theme-primary-500" wire:click="{{ $this->hasAllSelected ? 'deselectAllNotifications' : 'selectAllNotifications' }}">
                        @if($this->hasAllSelected)
                            <div class="flex justify-center items-center w-5 h-5 text-white rounded bg-theme-success-600">
                                <x-cauri-icon name="check-mark-bold" size="2xs" />
                            </div>
                        @else
                ...
                <div class="relative">
                    <x-cauri-dropdown wrapper-class="inline-block" dropdown-classes="mt-3" button-class="flex justify-center items-center p-4 h-10 dropdown-button-outline dark:border-theme-secondary-800">
                        @slot('button')
                            <div class="inline-flex justify-center items-center space-x-2 w-full">
                                <span class="w-full font-semibold text-left text-theme-secondary-900 dark:text-theme-secondary-200">
                ...
                <div class="w-10 sm:relative">
                    <x-cauri-dropdown wrapper-class="inline-block top-0 right-0 text-left sm:absolute" dropdown-classes="left-0 w-64 mt-3" button-class="flex justify-center w-10 h-10 rounded bg-theme-primary-100 text-theme-primary-600 dark:bg-theme-secondary-800 dark:text-theme-secondary-200">
                        <div class="py-3">
                            <button
                                type="button"
        ...
        @if ($notificationCount > 0 && $this->notifications->count() > 0)
            @foreach($this->notifications as $notification)
                    ...
                    >
                        <div class="flex flex-shrink-0 justify-between">
                            ...
                            <div class="flex items-center space-x-2 sm:hidden">
                                ...
                                >
                                    <x-cauri-icon name="trash" size="sm" />
                                </button>
                            </div>
                        </div>
                                    ...
                                    <div class="flex justify-between space-x-3">
                                        <div class="block space-x-3">
                                            ...
                                            @if ($notification->is_starred)
                                                ...
                                                >
                                                    <x-cauri-icon name="star" size="sm" class="text-theme-warning-200" />
                                                </button>
                                            @else
                                                <button
                                                    type="button"
                                                ...
                                                >
                                                    <x-cauri-icon name="star" size="sm" class="text-theme-secondary-300" />
                                                </button>
                                            @endif
                                        </div>
                ...
                @if (! $loop->last)
                    <div class="sm:px-10">
                        <hr class="mt-2 border-b border-dashed border-theme-secondary-200 dark:border-theme-secondary-800" />
                    </div>
                @endif
            @endforeach
            ...
            @if ($notificationCount > $this->notifications->perPage())
                <div class="flex justify-center mt-5">
                    {{ $this->notifications->links('vendor.cauri.pagination') }}
                </div>
            @endif
        @else
            <div class="flex flex-col justify-between items-center p-4 mt-5 space-y-2 rounded border-2 cursor-pointer sm:flex-row sm:space-y-0 border-theme-secondary-200 dark:border-theme-secondary-800">
                <span class="p-3">
                    @if (CauriLand\Foundation\Hermes\Enums\NotificationFilterEnum::isAll($this->activeFilter))
                ...
                @if (! CauriLand\Foundation\Hermes\Enums\NotificationFilterEnum::isAll($this->activeFilter))
                    ...
                    >
                        <x-cauri-icon name="arrows.arrow-rotate-left" />

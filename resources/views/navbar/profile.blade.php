...
>
    ...
    @foreach ($profileMenu as $menuItem)
                ...
                >
                    <div class="flex items-center space-x-3">
                        @if($menuItem['icon'] ?? false)
                            <x-cauri-icon :name="$menuItem['icon']" class="inline" />
                        @endif
        ...
        @else
            ...
            >
                <div class="flex items-center space-x-3">
                    @if($menuItem['icon'] ?? false)
                        <x-cauri-icon :name="$menuItem['icon']" calss="inline" />
                    @endif
    ...
    @if ($showDarkMode)
        <div class="my-4 mx-8">
            <x-cauri-divider color-class="bg-theme-secondary-300 text-theme-secondary-300 dark:bg-theme-secondary-700" />
        </div>
        ...
        >
            <div class="flex items-center space-x-2 font-medium cursor-default text-theme-secondary-900 dark:text-theme-secondary-200">
                <x-cauri-icon name="moon-underline" />

@php
    ...
    $twoAuthLink1 = view('cauri::external-link', [
    ...
    $twoAuthLink2 = view('cauri::external-link', [
        'attributes' => new ComponentAttributeBag([]),
...
<div dusk="two-factor-authentication-form">
    ...
    @else
        <div class="flex flex-col">
            ...
            <div class="flex flex-col items-center mt-4 space-y-4 sm:flex-row sm:items-start sm:mt-8 sm:space-y-0 sm:space-x-6">
                <div class="w-21 h-21">
                    <x-cauri-icon name="image.2fa" class="w-21 h-21 light-dark-icon" />
                </div>
    ...
    <div dusk="recovery-codes-modal">
        @if($this->modalShown)
            <x:cauri-fortify::modals.2fa-recovery-codes />
        @endif
    </div>

    @if($this->confirmPasswordShown)
        <x:cauri-fortify::modals.password-confirmation
